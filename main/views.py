...
from .models import Cluster, KubeConfig, AIConfig, SmtpConfig, SsoConfig, UserProfile
...
def has_permission(user, permission):
    ...
    try:
        ...
        if profile.role == 'admin':
            return True
        return getattr(profile, f'can_{permission}', False)
    ...
    if request.method == 'POST':
        try:
            ...
            if target_user.is_superuser:
                return JsonResponse({'status': 'error', 'message': 'Cannot restrict a superuser.'}, status=400)

            profile, _ = UserProfile.objects.get_or_create(user=target_user)
            profile.can_workloads          = 'workloads'          in permissions
            profile.can_cluster_management = 'cluster_management' in permissions
            profile.can_services           = 'services'           in permissions
            profile.can_storage            = 'storage'            in permissions
            profile.can_ingress            = 'ingress'            in permissions
            profile.can_configmaps         = 'configmaps'         in permissions
            profile.can_metrics            = 'metrics'            in permissions
            profile.can_events             = 'events'             in permissions
            profile.can_rbac               = 'rbac'               in permissions
            profile.save()

            logger.info(f"Permissions updated for {target_user.username} by {request.user.username}: {permissions}")
            return JsonResponse({'status': 'success', 'message': f'Permissions updated for {target_user.username}'})
    ...
    if request.user.is_superuser:
        for i, user in enumerate(User.objects.all().order_by('id'), start=1):
            ...
            users_data.append({
                ...
                'role': profile.role,
                'permissions': profile.get_permissions_display(),
                'can_workloads':          profile.can_workloads,
                'can_cluster_management': profile.can_cluster_management,
                'can_services':           profile.can_services,
                'can_storage':            profile.can_storage,
                'can_ingress':            profile.can_ingress,
                'can_configmaps':         profile.can_configmaps,
                'can_metrics':            profile.can_metrics,
                'can_events':             profile.can_events,
                'can_rbac':               profile.can_rbac,
            })
...
def google_callback(request):
    ...
    try:
        ...
        token_response = http_requests.post('https://oauth2.googleapis.com/token',
            data={'client_id': sso.client_id, 'client_secret': sso.client_secret,
                  'code': code, 'redirect_uri': redirect_uri, 'grant_type': 'authorization_code'}, timeout=10)
        token_data = token_response.json()
        access_token = token_data.get('access_token')
        if not access_token:
            return render(request, 'main/login.html', {'error_message': 'Failed to get access token from Google'})
        user_data = http_requests.get('https://www.googleapis.com/oauth2/v2/userinfo',
        ...
        login(request, user)
        kube_config_entry = KubeConfig.objects.first()
        return redirect('/KubeBuddy' if kube_config_entry and os.path.isfile(kube_config_entry.path) else '/integrate')
    except Exception as e:
...
def google_login_api(request):
    if request.method == 'POST':
        try:
            ...
            redirect_uri = sso.redirect_uri or (request.build_absolute_uri('/').rstrip('/') + '/google-callback/')
            token_data = http_requests.post('https://oauth2.googleapis.com/token',
                data={'client_id': sso.client_id, 'client_secret': sso.client_secret,
                      'code': code, 'redirect_uri': redirect_uri, 'grant_type': 'authorization_code'}, timeout=10).json()
            access_token = token_data.get('access_token')
            if not access_token:
            ...
            if request.session.session_key:
                response.set_cookie('sessionid', request.session.session_key, max_age=1209600, httponly=True, samesite='Lax')
            return response
        except Exception as e:
...
def github_callback(request):
    ...
    try:
        ...
        token_data = http_requests.post('https://github.com/login/oauth/access_token',
            headers={'Accept': 'application/json'},
            data={'client_id': sso.client_id, 'client_secret': sso.client_secret, 'code': code, 'redirect_uri': redirect_uri}, timeout=10).json()
        access_token = token_data.get('access_token')
        if not access_token:
            return render(request, 'main/login.html', {'error_message': 'Failed to get access token from GitHub'})
        user_data = http_requests.get('https://api.github.com/user',
            headers={'Authorization': f'token {access_token}', 'Accept': 'application/json'}, timeout=10).json()
        email = user_data.get('email')
        if not email:
            emails = http_requests.get('https://api.github.com/user/emails',
                headers={'Authorization': f'token {access_token}', 'Accept': 'application/json'}, timeout=10).json()
            email = next((e['email'] for e in emails if e.get('primary') and e.get('verified')), None) or next((e['email'] for e in emails), None)
        if not email:
    ...
    if request.method == 'POST':
        try:
            ...
            if not email:
                ...
                email = next((e['email'] for e in emails if e.get('primary') and e.get('verified')), None) or next((e['email'] for e in emails), None)
            if not email:
                return JsonResponse({'error': 'Could not retrieve email from GitHub.'}, status=400)
            name = user_data.get('name') or user_data.get('login', '')
            user, created = User.objects.get_or_create(email=email, defaults={
                'username': user_data.get('login', email.split('@')[0]),
                'first_name': name.split()[0] if name else '',
...
def outlook_callback(request):
    ...
    try:
        ...
        token_data = http_requests.post('https://login.microsoftonline.com/common/oauth2/v2.0/token',
            data={'client_id': sso.client_id, 'client_secret': sso.client_secret, 'code': code,
                  'redirect_uri': redirect_uri, 'grant_type': 'authorization_code', 'scope': 'openid email profile User.Read'}, timeout=10).json()
        access_token = token_data.get('access_token')
        if not access_token:
            return render(request, 'main/login.html', {'error_message': 'Failed to get access token from Microsoft'})
        user_data = http_requests.get('https://graph.microsoft.com/v1.0/me',
            headers={'Authorization': f'Bearer {access_token}', 'Accept': 'application/json'}, timeout=10).json()
